...
Also worth knowing: the Claude Code provider parses `claude -p "/usage"`'s human-readable text output, and the Codex CLI provider talks JSON-RPC to `codex app-server` (see [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)) — both read a real but undocumented-as-stable interface, so if either tool changes its output/protocol shape, please open an issue with the new format.
